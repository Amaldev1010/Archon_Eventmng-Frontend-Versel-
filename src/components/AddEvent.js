...
function EnhancedAddEvent() {
  ...
  const handleSendMessage = async (eventId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/events/${eventId}/send-message/`, {
        method: "POST",
        headers: {
  ...
  const handleAddOrUpdateEvent = async (e) => {
    ...
    const url = editingEventId
      ? `${API_BASE}/api/events/edit/${editingEventId}/`
  ...
  const handleDeleteEvent = async (eventId) => {
    try {
      const res = await fetch(`${API_BASE}/api/events/delete/${eventId}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${accessToken}` },
      })
  ...
  const styles = {
    ...
    // FIX: removed duplicate `border: "none"` — keeping only the visible border value
    button: {
      display: "flex",
      alignItems: "center",
      gap: "10px",
    ...
    secondaryButton: {
      ...
      fontWeight: "700",
      fontSize: "17px",
      boxShadow: `0 10px 30px rgba(239, 68, 68, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.2)`,
      border: "none",
      cursor: "pointer",
      transition: "all 0.4s cubic-bezier(0.4, 0, 0.2, 1)",
      animation: "gradientShift 3s ease-in-out infinite",
    ...
    eventCard: {
      ...
      borderRadius: "24px",
      padding: "32px",
      boxShadow: `0 15px 45px rgba(139, 92, 246, 0.08), inset 0 1px 0 rgba(255, 255, 255, 0.8)`,
      border: "1px solid rgba(139, 92, 246, 0.1)",
      transition: "all 0.4s cubic-bezier(0.4, 0, 0.2, 1)",
      animation: "slideUpScale 1s ease-out",
    },
      ...
      <style>
        {`
          ...
          input:focus, textarea:focus {
            ...
            box-shadow: 0 0 0 4px rgba(139, 92, 246, 0.1), 0 0 30px rgba(139, 92, 246, 0.15), inset 0 2px 10px rgba(139, 92, 246, 0.05);
            background: linear-gradient(135deg, rgba(255,255,255,0.95) 0%, rgba(248,250,252,0.8) 100%);
            transform: translateY(-2px);
          }
          .event-card:hover { transform: translateY(-12px) scale(1.02); box-shadow: 0 25px 60px rgba(139, 92, 246, 0.15); }
          .action-button:hover { transform: translateY(-3px) scale(1.05); box-shadow: 0 15px 40px rgba(139, 92, 246, 0.25); }
          .header-button:hover { transform: translateY(-2px); box-shadow: 0 10px 30px rgba(139, 92, 246, 0.15); }
          .primary-button:hover { transform: translateY(-4px) scale(1.02); box-shadow: 0 15px 40px rgba(139, 92, 246, 0.4); }
            ...
            <div style={styles.eventGrid}>
              {myEvents.map((event, index) => (
                <div
                  ...
                  style={{ ...styles.eventCard, animationDelay: `${index * 150}ms` }}
                >
                  ...
                  <div style={styles.eventActions}>
                    ...
                    <button onClick={() => handleViewParticipants(event.id)} className="action-button" style={{ ...styles.actionButton, ...styles.participantsButton }}>
